...
function s.tdfilter(c,e,tp,ft)
	return (c:IsSetCard(0x3008) or c:IsSetCard(0x1f)) 
		and c:IsAbleToDeck() and (ft>-1 or c:GetSequence()<5)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(s.tdfilter,tp,LOCATION_MZONE,0,1,nil,e,tp,ft) end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,s.tdfilter,tp,LOCATION_MZONE,0,1,1,nil,e,tp,ft):GetFirst()
...
function s.tdfilter2(c)
	return (c:IsSetCard(0x3008) or c:IsSetCard(0x1f)) and c:IsAbleToDeck()
end
function s.resfilter(c,sg)
	return c:IsSetCard(0x3008) and sg:IsExists(Card.IsSetCard,1,c,0x1f)
end
function s.rescon(sg,e,tp)
	return sg:IsExists(Card.IsCode,1,nil,CARD_NEOS) or sg:IsExists(s.resfilter,1,nil,sg)
end
function s.drtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local g=Duel.GetMatchingGroup(s.tdfilter2,tp,LOCATION_GRAVE,0,nil)
	if chk==0 then return aux.SelectUnselectGroup(g,e,tp,1,2,s.rescon,0) and Duel.IsPlayerCanDraw(tp,1) end
...
function s.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #sg>0 and Duel.SendtoDeck(sg,nil,SEQ_DECKSHUFFLE,REASON_EFFECT)==#sg then

...
function s.thfilter(c)
	return c:IsSetCard(SET_GOLD_PRIDE) and (c:IsMonster() and c:IsLocation(LOCATION_MZONE) and c:IsFaceup() or c:IsLocation(LOCATION_GRAVE)) and not c:IsCode(id) and c:IsAbleToHand()
end
...
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(s.spcfilter,tp,0,LOCATION_MZONE,1,nil,tp)
end
...
function s.spfilter(c,e,tp,code)
	return c:IsSetCard(SET_GOLD_PRIDE) and c:IsMonster() and c:ListsCode(code) and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end

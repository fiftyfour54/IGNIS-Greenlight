...
function s.spfilter(c,e,tp,pos,code)
	return c:IsRace(RACE_FISH) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,pos) and (not code or c:IsCode(code))
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_HAND|LOCATION_GRAVE,0,1,1,nil,e,tp,POS_FACEUP_DEFENSE)
	if #g1==0 or Duel.SpecialSummon(g1,0,tp,tp,false,false,POS_FACEUP_DEFENSE)==0 then return end
	local g2=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_GRAVE,0,nil,e,tp,g1:GetFirst():GetCode())

...
function s.silenforcfilter(c,e,tp,ft)
	return c:IsSetCard(SET_SILENFORC) and (c:IsAbleToHand() or (ft>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)))
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==1 then
		--Shuffle 1 LIGHT Warrior/Dragon Ritual Monster or 1 Ritual Spell into the Deck
		--and either add to the hand or Special Summon 1 "Silenforc" monster from the Deck
		...
		if Duel.SendtoDeck(tdg,nil,SEQ_DECKSHUFFLE,REASON_EFFECT)>0 then
			...
			if sc then
				aux.ToHandOrElse(sc,tp,function(c)
					return ft>0 and sc:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP) end,
				function(c)

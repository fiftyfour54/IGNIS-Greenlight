...
function s.setfilter(c,ec)
	return c:IsType(TYPE_TRAP) and c:IsSetCard(SET_ATTRACTION) and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE))
		and not c:IsCode(ec:GetCode()) and c:IsSSetable(true)
end
function s.cfilter(c,tp)
	return c:IsFaceup() and c:IsType(TYPE_TRAP) and c:IsSetCard(SET_ATTRACTION)

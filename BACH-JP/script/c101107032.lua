...
function s.initial_effect(c)
	c:EnableCounterPermit(COUNTER_PEACE)
	c:SetCounterLimit(COUNTER_PEACE,4)
...
function s.ctop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and c:AddCounter(COUNTER_PEACE,1) then
		local ct=c:GetCounter(COUNTER_PEACE)

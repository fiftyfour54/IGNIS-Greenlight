...
function s.filter(c)
	return c:IsType(TYPE_MONSTER) and c:IsRace(RACE_WINGEDBEAST) and c:IsAbleToRemoveAsCost()
end
function s.filteradd(c)
	return c:IsSetCard(0x16f) and (c:IsType(TYPE_MONSTER) or c:IsType(TYPE_FIELD)) and c:IsAbleToHand()
end

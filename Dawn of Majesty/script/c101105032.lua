...
function s.atkfilter(c)
	return c:IsType(TYPE_NORMAL) or c:IsSetCard(SET_MAGIKEY)
end
function s.atkcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return tc:GetBattleTarget()==c and not tc:IsStatus(STATUS_DISABLED) and tc:GetAttribute()&att~=0
end
function s.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chkc then return s.atkfilter(chkc,e,tp) and chkc:IsLocation(LOCATION_MZONE) end

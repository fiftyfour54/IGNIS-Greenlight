...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    ...
    if chkc then return chkc:IsControler(tp) and c:IsLocation(LOCATION_GRAVE) and c:IsSetCard(0x10) and chkc:IsCanBeSpecialSummoned(e,0,tp,false,false) and cancelcon(Group.FromCards(chkc)) end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local exg=Duel.GetMatchingGroup(s.filter,tp,LOCATION_EXTRA,0,nil,nil,tp)
    local cancelcon=s.rescon(exg)
	local g=aux.SelectUnselectGroup(mg,e,tp,1,2,cancelcon,chk,tp,HINTMSG_SPSUMMON,cancelcon)
    ...
    Duel.BreakEffect()
    local syng=Duel.GetMatchingGroup(s.filter(TYPE_SYNCHRO,Card.IsSynchroSummonable),tp,LOCATION_EXTRA,0,nil,g,tp,true)
    if #syng>0 then
...
function s.tgtg(e,c)
	return c:IsSetCard(0x10) and c:IsType(TYPE_SYNCHRO) and c:IsType(TYPE_MONSTER) and c~=e:GetHandler()
end
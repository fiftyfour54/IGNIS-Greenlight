...
function s.initial_effect(c)
	...
	e2:SetTarget(function(e,c) return c:IsSetCard(SET_SPYRAL) end)
	e2:SetValue(500)
	c:RegisterEffect(e2)
	...
	e3:SetTarget(function(e,c) return c:IsCode(41091257) end)
	c:RegisterEffect(e3)
end
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetLocationCount(1-tp,LOCATION_MZONE,tp)>0
		and c:IsCanBeSpecialSummoned(e,1,tp,false,false,POS_FACEUP_DEFENSE,1-tp)
		and #sdg>0 and #odg>0 and not sdg:GetFirst():IsPublic() and not odg:GetFirst():IsPublic() --If revealing is required
	end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,c,1,tp,0)
end
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,1,tp,1-tp,false,false,POS_FACEUP_DEFENSE)>0 then

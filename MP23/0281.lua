...
function s.tofieldtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:IsAbleToRemove() and Duel.IsExistingMatchingCard(s.tofieldfilter,tp,LOCATION_DECK,0,1,nil) end
...
function s.cfilter(c,e,tp,ft)
	return (c:IsSetCard(SET_NOBLE_ARMS) and c:IsAbleToHand())
		or (ft>0 and c:IsMonster() and c:IsSetCard(SET_ARTORIGUS) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP))
end
function s.tofieldop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and aux.RemoveUntil(c,nil,REASON_EFFECT,PHASE_STANDBY,id,e,tp,s.returnop) and c:IsLocation(LOCATION_REMOVED) then
		...
		local tc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.tofieldfilter),tp,LOCATION_DECK|LOCATION_HAND|LOCATION_GRAVE,0,1,1,nil):GetFirst()
		...
		if Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.cfilter),tp,LOCATION_DECK|LOCATION_GRAVE,0,1,nil,e,tp,ft) and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
			...
			aux.ToHandOrElse(sc,tp,
							function(sc) return ft>0 and sc:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP) end,

...
local locations=LOCATION_GRAVE|LOCATION_DECK|LOCATION_HAND
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetPossibleOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,locations)
end
function s.spfilter(c,e,tp)
	return c:IsRace(RACE_MACHINE) and c:IsSetCard(SET_TIME_THIEF)
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.spfilter),tp,locations,0,1,nil,e,tp)

...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--If normal summoned, special summon Level 7 earth warrior monster
...
function s.filter(c,e,tp)
	return c:GetLevel()==7 and c:IsRace(RACE_WARRIOR) and c:IsAttribute(ATTRIBUTE_EARTH) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP)
end
	--Activation legality
function s.sptg2(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	--Special summon a warrior from hand or GY
function s.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		if Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)>0 and g:GetFirst():IsCode(3167573) then

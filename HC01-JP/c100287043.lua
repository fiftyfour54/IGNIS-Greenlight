...
function s.thcfilter(c,tp)
	return c:IsReason(REASON_BATTLE+REASON_EFFECT) and c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) 
		...
		and (not tp or Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_GRAVE,0,1,nil,c:GetOriginalRace()))
end
function s.thfilter(c,rc)
	return c:IsType(TYPE_MONSTER) and c:IsOriginalRace(rc) and c:IsAbleToHand()
end
function s.thfilter2(c,eg)
	return c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and eg:IsExists(Card.IsOriginalRace,1,nil,c:GetOriginalRace())
end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	local dg=eg:Filter(s.thcfilter,nil)

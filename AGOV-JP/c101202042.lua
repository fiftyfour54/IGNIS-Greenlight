...
function s.checkfilter(c,tp)
	return c:IsPreviousLocation(LOCATION_EXTRA) and c:IsSummonPlayer(tp)
end
function s.checkop(e,tp,eg,ep,ev,re,r,rp)
	for p=0,1 do
		local gc=eg:FilterCount(s.checkfilter,nil,p)
		s.sum_count[p]=s.sum_count[p]+gc
		if s.sum_count[p]>=2 and Duel.GetFlagEffect(1-p,id)==0 then
			Duel.RegisterFlagEffect(1-p,id,RESET_PHASE|PHASE_END,0,2)
		end
	end
end

...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local op=Duel.SelectEffect(tp,
		{b1,aux.Stringid(id,3)}, --"Search 1 Level 1 Ritual monster or Recipe card
		{b2,aux.Stringid(id,4)}) --Banish "Recipe" cards and Special Summon 1 "Nouvelles" Ritual monster
	e:SetLabel(op)
	if op==1 then
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif op==2 then
		...
		local rmg=aux.SelectUnselectGroup(rg,e,tp,1,#rg,s.rescon,1,tp,HINTMSG_REMOVE,nil,s.rescon)
		...
		if #rmg>0 and Duel.Remove(rmg,nil,REASON_EFFECT)>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
			...
			local g=Duel.SelectMatchingCard(tp,s.spfilter2,tp,LOCATION_HAND,0,1,1,nil,e,tp,#rmg)
			if #g>0 then
...
function s.rescon(sg,e,tp,mg)
	return Duel.IsExistingMatchingCard(s.spfilter2,tp,LOCATION_HAND,0,1,nil,e,tp,#sg)
end

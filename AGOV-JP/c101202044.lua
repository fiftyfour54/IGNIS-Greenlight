...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	local e3=e2:Clone() --Maybe use a global registration to avoid double prompt (See Qixing Longyuan)
...
function s.cfilter(c,tp,zone)
	...
	return (zone&seq)>0 and (c:IsLocation(LOCATION_MZONE) and c:IsRace(RACE_DINOSAUR) and c:IsFaceup())
		or ((rac&RACE_DINOSAUR)>0 and (pos&POS_FACEUP)>0)
end

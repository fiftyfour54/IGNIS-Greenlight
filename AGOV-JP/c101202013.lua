...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Add to the hand or shuffle into the Deck 2 cards
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.GetMatchingGroup(s.filter,tp,LOCATION_GRAVE|LOCATION_REMOVED,LOCATION_GRAVE|LOCATION_REMOVED,nil,e)
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local b2=g:FilterCount(Card.IsAbleToDeck,nil)==#g
	if b1 or b2 then
		local op=Duel.SelectEffect(tp,
			{b1,aux.Stringid(id,0)}, --"Add both to the hand"
			{b2,aux.Stringid(id,1)}) --"Shuffle both into the Deck"
		if op==1 then

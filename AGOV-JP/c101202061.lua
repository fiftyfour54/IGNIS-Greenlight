...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--Add 1 card to the hand or Set 1 "Stars Align across the Milky Way"
...
function s.thfilter(c)
	return c:IsFaceup() and ((c:IsMonster() and c:IsType(TYPE_SPIRIT)) or c:IsRitualSpell()) and c:IsAbleToHand()
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	local op=e:GetLabel()
	if op==1 then
		--Add to the hand 1 Spirit monster or 1 Ritual Spell
	...
	elseif op==2 then
		--Set 1 "Stars Align across the Milky Way"

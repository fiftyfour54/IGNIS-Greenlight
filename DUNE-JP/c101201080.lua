...
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetCustomActivityCount(id,tp,ACTIVITY_CHAIN)+Duel.GetCustomActivityCount(id,1-tp,ACTIVITY_CHAIN)>=10
end
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0,0,3000,3000,10,RACE_CYBERSE,ATTRIBUTE_DARK) end
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Destroy(g,REASON_EFFECT)>0 and c:IsRelateToEffect(e)
		and c:IsLocation(LOCATION_MZONE) and c:IsSSetable(true) then
		Duel.BreakEffect()
		Duel.SSet(tp,c,tp,false)
	end
end
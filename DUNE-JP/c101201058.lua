...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x18c) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,0,LOCATION_DECK)
	Duel.SetOperationInfo(0,CATEGORY_RELEASE,nil,1,0,0,tp,LOCATION_MZONE+LOCATION_HAND)
end
function s.trfilter(c)
	return c:IsSetCard(0x18c) and c:IsReleasableByEffect()
end
...
function s.setfilter(c)
	return c:IsSetCard(0x18c) and c:IsTrap() and c:IsSSetable()
end

...
s.fit_monster={CARD_LIBROMANCER_FIREBURST,CARD_LIBROMANCER_FIRESTART}
function s.ritualfil(c)
	return c:IsSetCard(0x17d) and c:IsRitualMonster()
end
function s.mfilter(c)
	return c:IsPreviousLocation(LOCATION_MZONE) and c:GetPreviousCodeOnField()==CARD_LIBROMANCER_FIRESTART
end
function s.stage2(mg,e,tp,eg,ep,ev,re,r,rp,tc)
	if tc:IsCode(CARD_LIBROMANCER_FIREBURST) and mg:IsExists(s.mfilter,1,nil) then

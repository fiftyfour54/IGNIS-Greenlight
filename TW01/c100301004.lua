...
function s.icefilter(c)
	return c:IsFaceup() and c:IsSetCard(SET_ICE_BARRIER) and c:IsAbleToHand()
end
...
function s.retop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.icefilter,tp,LOCATION_MZONE,0,1,1,nil)
	if #g>0 then
		Duel.HintSelection(g,true)
		if Duel.SendtoHand(g,nil,REASON_EFFECT)>0 and g:GetFirst():IsLocation(LOCATION_HAND|LOCATION_DECK|LOCATION_EXTRA) then
...
function s.selfdesop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>=3 and aux.SelectUnselectGroup(g,e,tp,3,3,aux.dncheck,0,tp) then

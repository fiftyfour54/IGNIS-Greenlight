...
function s.counterfilter(c)
	return (c:IsCode(TOKEN_BRAVE) or aux.IsCodeListed(c,TOKEN_BRAVE))
end
...
function s.splimit(e,c,sump,sumtype,sumpos,targetp,se)
	return not (c:IsCode(TOKEN_BRAVE) or aux.IsCodeListed(c,TOKEN_BRAVE))
end
...
function s.tkop(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsPlayerCanSpecialSummonMonster(tp,TOKEN_BRAVE,0,TYPES_TOKEN,2000,2000,4,RACE_FAIRY,ATTRIBUTE_EARTH) then return end
	...
	if Duel.SpecialSummon(token,0,tp,player,false,false,POS_FACEUP)>0 and Duel.IsExistingMatchingCard(Card.IsAbleToGrave,tp,LOCATION_HAND,0,1,nil) then
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return (Duel.GetLocationCount(tp,LOCATION_MZONE)+Duel.GetLocationCount(1-tp,LOCATION_MZONE))>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,TOKEN_BRAVE,0,TYPES_TOKEN,2000,2000,4,RACE_FAIRY,ATTRIBUTE_EARTH) end
	Duel.SetOperationInfo(0,CATEGORY_TOKEN,nil,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,0)
end

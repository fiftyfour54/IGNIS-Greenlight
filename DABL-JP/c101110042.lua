...
function s.spfilter1(c,tp)
	return ((c:IsType(TYPE_SYNCHRO) and c:IsType(TYPE_TUNER)) and (c:IsFaceup() or not c:IsOnField()) and c:IsAbleToRemoveAsCost() and aux.SpElimFilter(c,true,true))
end
function s.spfilter2(c,tp)
	return c:IsCode(CARD_BLACK_WINGED_DRAGON) and (c:IsFaceup() or not c:IsOnField()) and c:IsAbleToRemoveAsCost() and aux.SpElimFilter(c,true) 
end
function s.chk(c,sg)
	return ((c:IsType(TYPE_SYNCHRO) and c:IsType(TYPE_TUNER)) and (c:IsFaceup() or not c:IsOnField()) and c:IsAbleToRemoveAsCost() and aux.SpElimFilter(c,true,true)) and sg:IsExists(s.spfilter2,1,c)
end

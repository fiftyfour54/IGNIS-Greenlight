...
function s.tgfilter(c,tp,check)
	return c:IsFaceup() and c:IsSetCard(0x24b) and Duel.IsExistingMatchingCard(s.eqfilter,tp,LOCATION_EXTRA+LOCATION_MZONE+LOCATION_GRAVE,0,1,nil,c,check)
end
function s.eqfilter(c,tc,check)
	return not c:IsForbidden() and c:CheckEquipTarget(tc) and c:IsType(TYPE_MONSTER) and ((c:IsSetCard(0x24b) and not c:IsLevel(4)) or check)
end
function s.chkfilter(c,typ)
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x24b) and c:IsType(typ) and c:IsFaceup()
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local check=Duel.IsExistingMatchingCard(s.chkfilter,tp,LOCATION_GRAVE,0,1,TYPE_FUSION)

...
function s.excvtop(e,tp,eg,ep,ev,re,r,rp)
	...
	local ct=0
	if Duel.GetLocationCount(tp,LOCATION_SZONE)>0 and g:IsExists(s.setfilter,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
...
function s.tgcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return Duel.GetTurnCount()~=e:GetLabel() and tc:GetFlagEffect(id)~=0-- and tc:IsOnField() and tc:IsFacedown() --tb checked
end

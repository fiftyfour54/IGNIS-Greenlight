...
function s.initial_effect(c)
	--The next damage you take becomes 0, discard 1 card and Special Summon 1 "Purery" monster
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	e2:SetCondition(function(e) return Duel.GetFlagEffect(tp,e:GetHandler():GetFieldID())==0 end)
...
function s.damval(e,re,val,r,rp,rc)
	...
	if r&(REASON_BATTLE|REASON_EFFECT)~=0 and Duel.GetFlagEffect(tp,fid)==0 then
		Duel.RegisterFlagEffect(tp,fid,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
...
function s.drwcond(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsTurnPlayer(1-tp) and e:GetHandler():IsSetCard(SET_PURERY)
end

...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.ChangePosition(tc,POS_FACEUP_DEFENSE,POS_FACEDOWN_DEFENSE,POS_FACEUP_ATTACK,POS_FACEUP_ATTACK)~=0 then
		local ct=Duel.GetMatchingGroup(aux.FilterFaceup(Card.IsSetCard,SET_VANQUISH_SOUL),tp,LOCATION_MZONE,0,nil):GetClassCount(Card.GetCode)
		local g=Duel.GetMatchingGroup(aux.FilterFaceup(Card.IsCanTurnSet),tp,0,LOCATION_MZONE,nil)
		if ct>0 and #g>0 and Duel.SelectEffectYesNo(tp,e:GetHandler(),aux.Stringid(id,1)) then
			Duel.BreakEffect()
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_POSCHANGE)

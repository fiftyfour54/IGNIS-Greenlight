...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if not tc:IsRelateToEffect(e) or ft<=0 then return end
	local g=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_HAND,0,nil,e,tp,tc:GetOriginalAttribute())
	if #g==0 then return end
	local sg=aux.SelectUnselectGroup(g,e,tp,1,math.min(2,ct),aux.dncheck,1,tp,HINTMSG_SPSUMMON)
	...
	if #og>0 then
		...
		if #thg>0 and Duel.SelectEffectYesNo(tp,e:GetHandler(),aux.Stringid(id,1)) then
			Duel.BreakEffect()

...
function s.disfilter(c,cc)
	return c:IsAttribute(ATTRIBUTE_EARTH) and c:IsDiscardable()
		and Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK,0,1,nil,cc:GetRace(),c:GetRace())
end
...
function s.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:IsDiscardable()	and Duel.IsExistingMatchingCard(s.disfilter,tp,LOCATION_HAND,0,1,c,c) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DISCARD)
	local g=Duel.SelectMatchingCard(tp,s.disfilter,tp,LOCATION_HAND,0,1,1,c,c)
	g:AddCard(c)
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	local race1, race2=dg:GetFirst():GetRace(), dg:GetNext():GetRace()

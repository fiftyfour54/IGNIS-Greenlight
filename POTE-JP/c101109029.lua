...
function s.posop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)>0
		and tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsLocation(LOCATION_MZONE)
		and tc:IsControler(tp) and Duel.ChangePosition(tc,POS_FACEDOWN_DEFENSE)>0 then
...
function s.lvrescon(c,mustlv)
	return function(sg)
		local sum=sg:GetSum(Card.GetLevel)
		return (sum==2 or sum==4) and (not mustlv or mustlv==sum)
			and c:IsLevelAbove(sum+1) and aux.dncheck(sg)
	end
end
function s.lvtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		return ft>0 and aux.SelectUnselectGroup(g,e,tp,1,ft,s.lvrescon(c),0)
	end
...
function s.lvop(e,tp,eg,ep,ev,re,r,rp)
	...
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT) then ft=math.min(1,ft) end
	if ft<1 or c:IsFacedown() or not c:IsRelateToEffect(e) or c:IsImmuneToEffect(e) then return end
	local g=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.lvfilter),tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,nil,e,tp)
	local lvs={}
	if aux.SelectUnselectGroup(g,e,tp,1,ft,s.lvrescon(c,2),0) then table.insert(lvs,2) end
	if aux.SelectUnselectGroup(g,e,tp,1,ft,s.lvrescon(c,4),0) then table.insert(lvs,4) end
	if #lvs<1 then return end
	local lv=Duel.AnnounceNumber(tp,table.unpack(lvs))
	if c:UpdateLevel(-lv)~=-lv then return end
	local tg=aux.SelectUnselectGroup(g,e,tp,1,ft,s.lvrescon(c,lv),1,tp,HINTMSG_SPSUMMON)
	if #tg>0 then
		Duel.SpecialSummon(tg,0,tp,tp,false,false,POS_DEFENSE)
	end
end
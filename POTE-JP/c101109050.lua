...
function s.matfilter(c,sc,st,tp)
	return c:IsControler(tp) and c:IsInMainMZone()
		and (c:IsSetCard(0x17c,sc,st,tp) or c:IsSummonCode(sc,st,tp,CARD_VISAS_STAFROST))
end
function s.thcon(e)
...
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsCode,CARD_VISAS_STAFROST),tp,LOCATION_MZONE,0,1,nil)
end

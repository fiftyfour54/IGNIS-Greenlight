...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
		...
		g=Duel.GetMatchingGroup(s.thfilter,tp,LOCATION_DECK,0,nil)
		if #g>0 and Duel.GetFieldGroupCount(tp,0,LOCATION_MZONE)>0 and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
			Duel.BreakEffect()
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
...
function s.gyfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x146) and c:IsType(TYPE_RITUAL)
end
...
function s.gyop(e,tp,eg,ep,ev,re,r,rp)
	...
	if (#g1<1 and #g2<1) or not e:GetHandler():IsRelateToEffect(e) then return end

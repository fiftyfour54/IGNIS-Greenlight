...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local b3=Duel.IsExistingMatchingCard(Card.IsFaceup,tp,0,LOCATION_ONFIELD,1,nil) and ct>=7
	local flag=0
	if b1 and ((not b2 and not b3) or Duel.SelectYesNo(tp,aux.Stringid(id,5))) then
	...
	if b2 and ((flag==0 and not b3) or Duel.SelectYesNo(tp,aux.Stringid(id,6))) then
		Duel.BreakEffect()
	...
	if b3 and (flag==0 or Duel.SelectYesNo(tp,aux.Stringid(id,7))) then
		Duel.BreakEffect()

...
function s.spfilter(c,e,tp,sanc)
	return ((c:IsSetCard(0x44) and not c:IsCode(id)) or (sanc and c:IsCode(55794644))) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local sanc=Duel.IsEnvironment(CARD_SANCTUARY_SKY)
			or Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsCode,CARD_SANCTUARY_SKY),e:GetHandlerPlayer(),LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil)

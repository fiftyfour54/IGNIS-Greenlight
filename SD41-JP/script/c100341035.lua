...
function s.eqfilter1(c,tp)
	return c:IsFaceup() and c:IsSetCard(0x4093) and c:IsType(TYPE_EFFECT) 
		and Duel.IsExistingMatchingCard(s.eqfilter2,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,nil,c,tp)
end
...
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if ec and ec:IsRelateToEffect(e) and ec:IsFaceup() then
		...
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.eqfilter2),tp,LOCATION_GRAVE,LOCATION_GRAVE,1,1,nil,ec,tp)
		local tc=g:GetFirst()
...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetFlagEffect(tp,id+100)==0 and #mg>0 

...
function s.nonaquafilter(c)
	return c:IsAttackPos() and not c:IsRace(RACE_AQUA)
end
function s.aquafilter(c)
	return c:IsRace(RACE_AQUA) and c:IsFaceup()
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.aquafilter,tp,LOCATION_MZONE,0,1,nil)
		and Duel.IsExistingMatchingCard(s.nonaquafilter,tp,0,LOCATION_MZONE,1,nil) end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	--Effect
		local ct=Duel.GetMatchingGroupCount(s.aquafilter,tp,LOCATION_MZONE,0,nil)
		if ct==0 then return end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_POSCHANGE)
		local pg=Duel.SelectMatchingCard(tp,s.nonaquafilter,tp,0,LOCATION_MZONE,1,ct,nil)
		if #pg>0 then

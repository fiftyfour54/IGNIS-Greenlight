...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(FilterMaximumSideFunctionEx(s.recfilter),tp,LOCATION_MZONE,0,1,nil) end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	--Effect
	local g=Duel.SelectMatchingCard(tp,FilterMaximumSideFunctionEx(s.recfilter),tp,LOCATION_MZONE,0,1,1,nil)
	if #g>0 then

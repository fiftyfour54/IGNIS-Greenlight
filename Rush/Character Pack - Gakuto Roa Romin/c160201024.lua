...
function s.filter(c)
    return c:IsRace(TYPE_NORMAL) and c:IsType(TYPE_MONSTER) and c:IsRace(RACE_PSYCHIC) and c:IsLevel(1) and c:IsAbleToDeckAsCost()
end
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_GRAVE,0,1,nil) end
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
     local dg=Duel.GetMatchingGroup(s.filter,tp,0,LOCATION_MZONE,e:GetHandler())

...
function s.cfilter(c,tp)
	return c:IsReason(REASON_BATTLE|REASON_EFFECT) and c:IsOriginalAttribute(ATTRIBUTE_FIRE) 
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		if Duel.Destroy(tc,REASON_EFFECT)>0 and c:IsFaceup() and c:IsRelateToEffect(e) then
			c:UpdateAttack(c,atk,RESET_EVENT|RESETS_STANDARD|RESET_PHASE|PHASE_END)
		end
	end
end